...
scheduleRoute.post( "/", authMiddleware, validate(createDoctorScheduleSchema), asyncHandler(controller.createSchedule));
scheduleRoute.get( "/:doctorId", authMiddleware, asyncHandler(controller.getDoctorSchedule));
scheduleRoute.patch( "/:id", validate(updateDoctorScheduleSchema), authMiddleware, asyncHandler(controller.updateSchedule));
