...
export class AppointmentService {
   ...
   async createAppointment(userId: string, data: CreateAppointmentDto) {
      ...
      if (
         ...
         throw new AppError(
            "Appointment time is outside doctor working hours", 400,
      ...
      if (diff % slotDuration !== 0) {
         throw new AppError(
            "Invalid appointment slot", 400, "INVALID_APPOINTMENT_SLOT");
   ...
   async getAllAppointments(query: any) {
      ...
      const [appointment, total] = await this.appointmentRepository.findAll(skip, limit, statusId, sort, order, search);
